...
router.post('/start', apiRateLimit, ensureAuth, ensureRole(['guard']), [
    body('templateId').isInt()
], validateRequest, patrolController.startPatrol);
router.post('/scan', apiRateLimit, ensureAuth, ensureRole(['guard']), [
    body('runId').isInt(),
    body('checkpointId').isInt(),
    body('location').optional()
], validateRequest, patrolController.scanCheckpoint);
router.post('/end', apiRateLimit, ensureAuth, ensureRole(['guard']), [
    body('runId').isInt()
], validateRequest, patrolController.endPatrol);
